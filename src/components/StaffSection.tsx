...
const StaffSection = ({ title, subtitle, cargo, level, section }: Props) => {
  ...
  const groups = useMemo(() => {
    ...
    for (const t of teachers) {
      ...
      const gradeGroups = map.get(grade)!;

      let sectionGroup = gradeGroups.find(
        (g) => g.section === section
      );
    ...
    <section className="pt-24 md:pt-28 pb-20">
      <div className="container mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          <span className="text-accent font-semibold uppercase tracking-widest text-lg">Nuestro Equipo</span>
            ...
            <div className="flex justify-center mb-14">
              {coord ? (
                <div className="w-full max-w-xs">
                  <PersonCard photo={coord.photo_url} title={coord.name} subtitle={cargo} highlight />
                </div>
              ) : (
              ...
              <div className="space-y-10 max-w-6xl mx-auto">
                {groups.map(([grade, sections]) => (
                  <div key={grade}>
                    ...
                    <h3 className="font-display text-xl font-bold text-accent mb-6 border-b border-border pb-2">
                      {grade}
                    </h3>
